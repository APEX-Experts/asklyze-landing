...
const SocialSprite = ({ offset }: { offset: number }) => (
    <svg width="38" height="38" viewBox={`${offset} 0 38 38`} fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <g clipPath="url(#clip5_0_8)">
            <path d="M180.485 13.2087L185.437 19.8311L180.454 25.213H181.576L185.938 20.5002L189.464 25.2133H193.28L188.049 18.2196L192.688 13.2087H191.566L187.548 17.5484L184.302 13.2087H180.485ZM182.135 14.035H183.888L191.63 24.3868H189.877L182.135 14.035Z" fill="#1F2A6B"/>
        </g>
        <g clipPath="url(#clip7_0_8)">
            <path d="M249.929 16.3246C249.849 16.0283 249.693 15.7581 249.476 15.5412C249.259 15.3242 248.989 15.168 248.693 15.0883C247.601 14.793 243.217 14.793 243.217 14.793C243.217 14.793 238.832 14.793 237.741 15.0883C237.445 15.168 237.175 15.3242 236.958 15.5412C236.741 15.7581 236.584 16.0283 236.505 16.3246C236.301 17.4374 236.202 18.5669 236.209 19.6981C236.202 20.8294 236.301 21.9588 236.505 23.0717C236.584 23.3679 236.741 23.6381 236.958 23.8551C237.175 24.072 237.445 24.2282 237.741 24.308C238.832 24.6033 243.217 24.6033 243.217 24.6033C243.217 24.6033 247.601 24.6033 248.693 24.308C248.989 24.2282 249.259 24.072 249.476 23.8551C249.693 23.6381 249.849 23.3679 249.929 23.0717C250.133 21.9588 250.231 20.8294 250.224 19.6981C250.231 18.5669 250.133 17.4374 249.929 16.3246ZM241.815 21.8004V17.5959L245.454 19.6981L241.815 21.8004Z" fill="#1F2A6B"/>
            <path d="M241.822 17.5918V21.8048L245.459 19.6983L241.822 17.5918Z" fill="white"/>
        </g>
        <defs>
            <clipPath id="clip3_0_8">
                <rect width="14" height="14.8235" fill="white" transform="translate(123.875 11.4592)"/>
            </clipPath>
            <clipPath id="clip5_0_8">
                <rect width="14" height="13.1765" fill="white" transform="translate(179.875 12.533)"/>
            </clipPath>
            <clipPath id="clip7_0_8">
                <rect width="15" height="10.5" fill="white" transform="translate(235.59 14.1211)"/>
            </clipPath>
        </defs>
    </svg>
);

const SocialIcon = ({ offset, href, label }: { offset: number, href: string, label: string }) => (
    <a href={href} aria-label={label} className="w-[38px] h-[38px] rounded-full bg-[#FAFAFA] flex items-center justify-center transition-all hover:-translate-y-1 shadow-sm overflow-hidden border border-black/5">
                ...
                <div className="w-full max-w-[1240px] px-6 mx-auto flex flex-col md:flex-row justify-between items-start gap-12 md:gap-8">
                    ...
                    <div className="flex flex-col gap-[24px] max-w-[384px]">
                        <Link href={`/${currentLocale}`} className="inline-block">
                            <img src="/images/asklyze-cyan-logo.svg" alt="Asklyze Logo" width={384} height={60} className="w-full max-w-[384px] h-auto object-contain" />
                        </Link>
                        ...
                        <div className="flex items-center gap-[18px] mt-2">
                            <SocialIcon offset={0} href={dict.social.facebook} label="Facebook" />
                            <SocialIcon offset={56} href={dict.social.linkedin} label="LinkedIn" />
                            <SocialIcon offset={112} href={dict.social.instagram} label="Instagram" />
                            <SocialIcon offset={168} href={dict.social.twitter} label="X (Twitter)" />
                            <SocialIcon offset={224} href={dict.social.youtube} label="Youtube" />
                        </div>
                    </div>

                    {/* Column 2: Legal */}
                    <div className="flex flex-col gap-[16px] w-[190px]">
                        <h3 className="text-[24px] font-semibold text-[#1F2A6B] leading-[1.21]">Legal</h3>
                        <Link href={`/${currentLocale}/privacy`} className="text-[18px] text-[#1A1A1A] opacity-70 hover:opacity-100 transition-opacity">
                            {dict.bottomLinks.privacy}
                        </Link>
                        <Link href={`/${currentLocale}/terms`} className="text-[18px] text-[#1A1A1A] opacity-70 hover:opacity-100 transition-opacity">
                            {dict.bottomLinks.terms}
                        </Link>
                        <Link href={`/${currentLocale}/security`} className="text-[18px] text-[#1A1A1A] opacity-70 hover:opacity-100 transition-opacity">
                    ...
                    <div className="flex flex-col gap-[16px] w-[120px]">
                        <h3 className="text-[24px] font-semibold text-[#1F2A6B] leading-[1.21]">Links</h3>
                        <Link href={`/${currentLocale}/about`} className="text-[18px] text-[#1A1A1A] opacity-70 hover:opacity-100 transition-opacity">
                            {dict.links.about}
                        </Link>
                        <Link href={`/${currentLocale}/blog`} className="text-[18px] text-[#1A1A1A] opacity-70 hover:opacity-100 transition-opacity">
                            Blogs
                        </Link>
                        <a href="https://docs.asklyze.ai/" target="_blank" rel="noopener noreferrer" className="text-[18px] text-[#1A1A1A] opacity-70 hover:opacity-100 transition-opacity">
                            Docs
                        </a>
                    </div>
                    ...
                    <div className="flex flex-col gap-[16px] max-w-[280px]">
                        <h3 className="text-[24px] font-semibold text-[#1F2A6B] leading-[1.21]">Location</h3>

...
export default function TermsAndConditionsContent({ lang, isArabic, content }: TermsAndConditionsContentProps) {
    const [activeSection, setActiveSection] = useState(0);
    const allSections = [...(content.sections || []), ...(content.additionalSections || [])];
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <section 
                ...
                style={{ 
                    background: "linear-gradient(180deg, rgba(250, 250, 250, 1) 0%, rgba(245, 245, 245, 1) 42%, rgba(238, 241, 255, 1) 100%)",
                    paddingTop: "150px",
                    paddingBottom: "80px", 
                }}
            >
            ...
            <main className="bg-white flex justify-center py-20 overflow-visible">
                <div className="w-full max-w-[1240px] px-6 flex flex-col md:flex-row justify-between items-start gap-10">
                    ...
                    {/* Left Sidebar Menu */}
                    <aside className="w-full md:w-auto md:min-w-[280px] md:max-w-[320px] flex-shrink-0 flex flex-col gap-[16px] sticky top-[120px]">
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center gap-[10px] text-left transition-colors text-[14px] leading-[1.3] font-normal capitalize ${
                                        isActive ? "text-[#1F2A6B]" : "text-[#6B6B6B] hover:text-[#181A2A]"
                                    }`}
                                >
                                    {isActive && (
                                        <div className="flex-shrink-0 w-[18px] h-[18px] flex items-center justify-center">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g clipPath="url(#clip0_336_3714)">
                                                    <path d="M17.6271 8.41573L0.913266 0.701637C0.647129 0.580783 0.329566 0.652781 0.144427 0.881633C0.0538343 0.992485 0.00307248 1.13054 0.000275068 1.27367C-0.00252234 1.4168 0.0428066 1.55674 0.128999 1.67104L5.62529 8.99943L0.128999 16.3278C-0.0484251 16.5631 -0.0419967 16.8897 0.143142 17.1172C0.267853 17.2728 0.454277 17.3564 0.643272 17.3564C0.734556 17.3564 0.825839 17.3371 0.91198 17.2972L17.6259 9.58313C17.8547 9.47771 18 9.25014 18 8.99943C18 8.74872 17.8547 8.52116 17.6271 8.41573Z" fill="#1F2A6B"/>
                                                </g>
                                                <defs>
                    ...
                    {/* Right Main Content */}
                    <div className="w-full max-w-[857px] flex flex-col gap-12 font-inter text-[14px] leading-[2] text-[#1A1A1A]">
                        {allSections.map((section: any, index: number) => (
                            <article
                                id={`section-${index}`}
                                data-index={index}
                                    ...
                                    <ul className="space-y-2 mb-4">
                                        {section.points.map((point: string, idx: number) => (
                                            <li key={idx} className="flex items-start gap-3">
                                                <span className="mt-[2px] shrink-0 text-[#1F2A6B]">•</span>
